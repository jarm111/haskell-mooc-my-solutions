...
add e p@(Set xs) = if member e p then p else Set (sort (e : xs))
...
passwordAllowed pw (ContainsSome xs) = length (intersect pw xs) > 0
passwordAllowed pw (DoesNotContain xs) = null (intersect pw xs)
