...
oddFactorial 1 = 1
oddFactorial 2 = 1
oddFactorial n = if even n then (n-1) * oddFactorial (n-3) else n * oddFactorial (n-2)
...
smallestDivisor n = handleResult n 2
  where handleResult n d = if n `mod` d == 0 then d else handleResult n (d + 1)
...
isPrime n = if n < 2 then False else n == smallestDivisor n
