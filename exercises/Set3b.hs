...
sums i = sums' i 2 1
  where sums' 0 _ _   = []
        sums' i j sum = sum : sums' (i - 1) (j + 1) (sum + j)
...
-- indexDefault xs i def = def
indexDefault xs i def = go xs i def 0
  where go [] _ def _ = def
        go (x:xs) i def current
          | i == current = x
          | otherwise    = go xs i def (current + 1)
...
sorted [] = True
sorted [x] = True
sorted (x:y:xs) = (x <= y) && sorted (y : xs) -- if x > y then False else sorted (y : xs)
...
maybeMap _ [] = []
maybeMap f (x:xs) = go (f x) f xs
  where go Nothing f xs   =  maybeMap f xs
        go (Just x) f xs  = x : maybeMap f xs
